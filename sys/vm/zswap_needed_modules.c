...
#include <vm/zswap_needed_modules.h>
...
{
    struct iovec iov[1];
    iov[0].iov_base = (void*) PHYS_TO_DMAP(VM_PAGE_TO_PHYS(linux_page_to_freebsd(page)));
    iov[0].iov_len = len;
    uio->uio_iov = iov;
    uio->uio_iovcnt = 1;
    uio->uio_offset = 0;
...
void uio_set_comp(struct uio* uio_out, void* buf, unsigned int buflen)
{
    ...
    uio_out->uio_iov = iov;
    uio_out->uio_iovcnt = 1;
    uio_out->uio_offset = 0;
    uio_out->uio_resid = buflen;
    uio_out->uio_segflg = UIO_SYSSPACE;
    //uio_rw暂时无法设置
    return;
}
